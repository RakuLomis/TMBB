...
def monitor_app_ports(app_name='chrome.exe', time_out=5000):
        ...
        while not stop_monitoring: # time_out is the time limitaion of one circle
            try:
                ...
                for event in creation_events:
                    on_process_creation(event, app_name)
...
def to_wireshark_filter(udp_ports, tcp_ports, begin_time, end_time): 
    ...
    # file_name = f"Papers\\UserPattern\\wireshark_order\\tshark\\{begin_time}--{end_time}.txt" 
    file_name = f"Papers\\UserPattern\\traffictracer\\tshark_port\\{begin_time}--{end_time}.txt"
...
def tsharkListening(pcap_path, device): # Start and end manually
    try: 
        ...
        print(f"Process {process.pid} starts successfully.")
...
def filter_pcap(input_pcap, output_pcap, display_filter): # filter process is been realized in log_analysis
    pass
...
if __name__ == "__main__": 
    ...
    pcap_directory = "traffictracer"
    device = 'WLAN'
    device_proxy = 'Meta'
    time = datetime.now().strftime("%y-%m-%d--%H-%M-%S")
    pcap_path_normal = os.path.join('Papers\\UserPattern\\', pcap_directory, device, 'WLAN-' + time + '.pcap')
    pcap_path_proxy = os.path.join('Papers\\UserPattern\\', pcap_directory, device_proxy, 'Meta-' + time + '.pcap')

    normal_t = tsharkListening(pcap_path_normal, device)
    proxy_t = tsharkListening(pcap_path_proxy, device_proxy)
    ...
    filter_order = to_wireshark_filter(all_udp_ports_set, all_tcp_ports_set, begin_time=time, end_time=end_time)
    print('Wireshark order has been generated.')

    tsharkTerminate(normal_t)
    tsharkTerminate(proxy_t)
